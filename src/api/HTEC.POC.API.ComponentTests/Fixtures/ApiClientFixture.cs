...
public abstract class ApiClientFixture : ApiFixture<Startup>
{
    ...
    /// <summary>
    /// Send a POST Http request to the API CreateRewards endpoint passing the rewards being created
    /// </summary>
    /// <param name="rewards">Rewards being created</param>
    public async Task<HttpResponseMessage> CreateRewards(CreateRewardsRequest rewards)
    {
        return await SendAsync(HttpMethod.Post, "/v1/rewards", rewards);
    }

    /// <summary>
    /// Send a POST Http request to the API CreateCategory endpoint passing the rewards id and category being created
    /// </summary>
    /// <param name="category">Category being created</param>
    public async Task<HttpResponseMessage> CreateCategory(Guid rewardsId, CreateCategoryRequest category)
    {
        return await SendAsync(HttpMethod.Post, $"/v1/rewards/{rewardsId}/category", category);
    }
