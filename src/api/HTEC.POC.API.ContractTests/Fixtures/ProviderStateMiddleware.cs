...
public class ProviderStateMiddleware : IMiddleware
{
    ...
    public ProviderStateMiddleware(IRewardsRepository repository)
    {
    ...
    private void ExistingRewards()
    {
        var rewards = new Fixture().Create<Rewards>();

        repository.GetByIdAsync(Arg.Any<Guid>()).Returns(Task.FromResult(rewards));
        repository.SaveAsync(Arg.Any<Rewards>()).Returns(true);
    }

    private void RewardsDoesNotExist()
    {
        repository.GetByIdAsync(Arg.Any<Guid>()).Returns(Task.FromResult((Rewards)null));
    }
