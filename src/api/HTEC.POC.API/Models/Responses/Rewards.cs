using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.Linq;
using Query = HTEC.POC.CQRS.Queries.GetRewardsById;

namespace HTEC.POC.API.Models.Responses;

/// <summary>
/// Response model used by GetById api endpoint
/// </summary>
public class Rewards
{
    /// <example>d290f1ee-6c54-4b01-90e6-d701748f0851</example>
    [Required]
    public Guid? Id { get; private set; }

    /// <example>Rewards name</example>
    [Required]
    public string Name { get; private set; }

    /// <example>Rewards description</example>
    public string Description { get; private set; }

    /// <summary>
    /// Represents the categories contained in the rewards
    /// </summary>
    public List<Category> Categories { get; private set; }

    /// <summary>
    /// Represents the status of the rewards. False if disabled
    /// </summary>
    [Required]
    public bool? Enabled { get; private set; }

    public static Rewards FromQuery(Query.Rewards rewards)
    {
        return new Rewards
        {
            Id = rewards.Id,
            Name = rewards.Name,
            Description = rewards.Description,
            Categories = rewards.Categories?.Select(Category.FromQuery).ToList(),
            Enabled = rewards.Enabled
        };
    }
}
