...
using HTEC.POC.API.Authorization;
using HTEC.POC.API.Models.Requests;

namespace HTEC.POC.API;
        ...
        IOptions<JwtBearerAuthenticationConfiguration> jwtBearerAuthenticationOptions)
    {
        ...
        app
            ...
            .UseSwaggerUI(c =>
            {
                ...
                c.SwaggerEndpoint("all/swagger.json", "Rewards (all)");
                c.SwaggerEndpoint("v1/swagger.json", "Rewards (version 1)");
                c.SwaggerEndpoint("v2/swagger.json", "Rewards (version 2)");
        ...
        services
            ...
            .AddSwaggerGen(c =>
            {
                c.SwaggerDoc("all", new OpenApiInfo
                {
                    Version = "all",
                    Title = "Rewards API",
                    Description = "APIs used to interact and manage rewardss for a restaurant",
                    Contact = new OpenApiContact()
                    {
                        Name = "Amido", Url = new Uri(projectUrl), Email = "stacks@amido.com"
                    },
                ...
                c.IncludeXmlComments(
                    $"{AppContext.BaseDirectory}{Path.DirectorySeparatorChar}{typeof(CreateRewardsRequest).Assembly.GetName().Name}.xml");

                //Set default tags, shows on top, non defined tags appears at bottom
                c.DocumentFilter<SwaggerDocumentTagger>(
                    new OpenApiTag[]
                    {
                        new OpenApiTag {Name = "Rewards"}, new OpenApiTag {Name = "Category"},
