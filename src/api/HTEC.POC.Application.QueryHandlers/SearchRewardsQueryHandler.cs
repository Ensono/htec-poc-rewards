...
public class SearchRewardsQueryHandler : IQueryHandler<SearchRewards, SearchRewardsResult>
{
    readonly IDocumentSearch<Rewards> storage;

    public SearchRewardsQueryHandler(IDocumentSearch<Rewards> storage)
    {
        this.storage = storage;
    }

    public async Task<SearchRewardsResult> ExecuteAsync(SearchRewards criteria)
    {
        if (criteria == null)
            throw new ArgumentException("A valid SearchRewardsQueryCriteria os required!");
        ...
        var result = new SearchRewardsResult();
        ...
        result.Results = results.Content.Select(i => new SearchRewardsResultItem()
        {
            Id = i.Id,
