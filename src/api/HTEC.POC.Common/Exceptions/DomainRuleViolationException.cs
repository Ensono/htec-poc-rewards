...
public class DomainRuleViolationException : ApplicationExceptionBase
{
    ...
    public static void Raise(OperationCode operationCode, Guid correlationId, Guid rewardsId, DomainExceptionBase domainException)
    {
        var exception = new DomainRuleViolationException(
            (ExceptionCode)domainException.ExceptionCode,
            operationCode,
            correlationId,
            $"A domain exception has been raised in the rewards '{rewardsId}'. {domainException.Message}",
            domainException
        );
    ...
    public static void Raise(IOperationContext context, Guid rewardsId, DomainExceptionBase domainException)
    {
        Raise((OperationCode)context.OperationCode, context.CorrelationId, rewardsId, domainException);
    }
}
