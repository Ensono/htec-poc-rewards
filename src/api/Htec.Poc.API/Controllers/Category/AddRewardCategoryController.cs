...
public class AddRewardCategoryController : ApiControllerBase
{
    readonly ICommandHandler<CreateCategory, Guid> commandHandler;

    public AddRewardCategoryController(ICommandHandler<CreateCategory, Guid> commandHandler)
    {
        this.commandHandler = commandHandler ?? throw new ArgumentNullException(nameof(commandHandler));
    }

    /// <summary>
    /// Create a category in the reward
    /// </summary>
    /// <remarks>Adds a category to reward</remarks>
    ...
    [HttpPost("/v1/reward/{id}/category/")]
    [Authorize]
    [ProducesResponseType(typeof(ResourceCreatedResponse), StatusCodes.Status201Created)]
    public async Task<IActionResult> AddRewardCategory([FromRoute][Required]Guid id, [FromBody]CreateCategoryRequest body)
    {
