...
public class CommandsAndQueriesTests
{
    ...
    {
        var assembly = typeof(CreateReward).Assembly;
    ...
    {
        var definitions = typeof(CreateReward).Assembly.GetImplementationsOf(typeof(ICommand));
    ...
    {
        var definitions = typeof(CreateReward).Assembly.GetImplementationsOf(typeof(IOperationContext));
    ...
    {
        var definitions = typeof(GetRewardById).Assembly.GetImplementationsOf(typeof(IQueryCriteria));
        foreach (var definition in definitions)
        {
            var operationCode = GetOperationCode(definition.implementation);
            var operationName = GetOperationName(operationCode);
    ...
    {
        var commands = typeof(CreateReward)
            .Assembly
            .GetImplementationsOf(typeof(ICommand))
            .Select(c => c.Item2);

        var handlers = typeof(CreateRewardCommandHandler)
            .Assembly
    ...
    {
        var queries = typeof(GetRewardById)
            .Assembly
            .GetImplementationsOf(typeof(IQueryCriteria))
            .Select(c => c.Item2);

        var handlers = typeof(GetRewardByIdQueryHandler)
            .Assembly
